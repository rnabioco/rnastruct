...
plot_sen_ppv <- function(plt_dat,
    ...
    # strip_font_size <- 12
    # p_sen <- p_sen + 
    #   theme(strip.text = element_text(size = strip_font_size))
    # 
    # p_ppv <- p_ppv + 
    #   theme(strip.text = element_text(size = strip_font_size))
...
plot_pred <- function(df, 
                      folding_dir = "no_slop/15_rnastruct_mapfiles",
                      outdir = "."){
  ...
  plot_secondary_structure(df$seq, 
                           ...
                           outpath = file.path(outdir,
                                               paste0(df$new_id, 
                                                      "_with_dms.png")),
                           windsorize = FALSE,
                           overlay_args = list(img_pos = c(0.85, 0.65))
                           )
...
to_plot <- c(
  "865959_0_+_100", #UBE2I_chr16:1375031-1375074
  "1022231_0_+_95", # CLTC CDS
  "1495619_0_+_59", # HMGXB4 3UTR
  "36778_0_-_88", # AK2_chr1:33490097-33490129
  "864505_0_+_136", #RAB40C_chr16:676003-676047
  "694318_1_-_74" #HECTD1_chr14:31604107-31604160
)
# to_plot <- c(
#   #"2002388.10_1_-_200",
#   "1098335_0_+_74",
...
plt_dat <- mutate(plt_dat, 
                  new_id = str_replace_all(new_id,
                                           "[[:punct:]]", "_"))
walk(split(plt_dat, 1:nrow(plt_dat)[1]),
...
```{r }
folding_dir <- "no_slop_ac"

outdir <- "plots/rnastructure/ac_nucs"
dir.create(outdir, recursive = TRUE)
...
```{r paths}
folding_dir <- "no_slop_tg"

outdir <- "plots/rnastructure/tg_nucs"
dir.create(outdir, recursive = TRUE)
