...
### WT
```{r}
m_files <- dir("wt_map", pattern = ".map$", full.names = T, recursive = TRUE) %>% 
  .[!str_detect(., "rRNA")]
...
```{r}
m_files <- dir("c4_map", pattern = ".map", full.names = T)
...
for(i in seq_along(dat)){
  ...
  plot_secondary_structure(seq = dat[[i]][[1]], 
                           ...
                           outpath = file.path(
                             outdir,
                             paste0(names(dat[i]), "_c4_depth15.png")),
                           overlay_args = list(img_pos = pos))
...
```{r}
m_files <- dir("rRNA/wt_map/", pattern = ".map", full.names = T)
