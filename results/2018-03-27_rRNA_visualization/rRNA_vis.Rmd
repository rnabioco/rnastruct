...
all_rrna_reactivity <- left_join(all_positions, 
  ...
  select(pos, value) %>% 
  mutate(value = ifelse(is.na(value) | value < 0.01, 
                        0,
                        1)) 
rrna_reactivity <- pull(all_rrna_reactivity, value) %>% .[1:450] %>% str_c(., collapse = ";")
```
